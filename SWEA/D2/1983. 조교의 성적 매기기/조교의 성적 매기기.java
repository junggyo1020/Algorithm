		...
		*/
		...
		for(int test_case = 1; test_case <= T; test_case++)
		{
            ...
            for(int i = 0; i < n; i++){
                ...
                totalScores[i] = midScore*(0.35) + finalScore*(0.45) + assignScore*(0.20);
                if(i==k){
                	targetScore = totalScores[i];
                }
            }
            
            Double[] sortedScores = Arrays.stream(totalScores).boxed().toArray(Double[]::new);
            Arrays.sort(sortedScores, Collections.reverseOrder());
            
            int rank = Arrays.asList(sortedScores).indexOf(targetScore);
